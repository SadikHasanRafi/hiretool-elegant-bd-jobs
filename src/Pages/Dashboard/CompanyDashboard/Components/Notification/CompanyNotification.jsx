...
const CompanyNotification = (props) => {
  ...
  useEffect(() => {
    const fetchEmployeeData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/get-single-employee-for-company?employeeUID=${appointmentEmployeeUID}`);
        const jobResponse = await axios.get(`http://localhost:5000/get-single-job?_id=${job_id}`);
