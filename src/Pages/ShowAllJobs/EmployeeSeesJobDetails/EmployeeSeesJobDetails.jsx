...
const EmployeeSeesJobDetails = () => {
    ...
    const fetchJobData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:5000/get-single-job-details-employee/${id}`
        );
        setJobDetails(response.data);
        const getCompanyDetails = await axios.get(
          `http://localhost:5000/get-company-for-employee/${response.data.uid}`
        );
        setCompanyDetails(getCompanyDetails.data);
        const getPostedJobsByThisCompanyk = await axios.get(
          `http://localhost:5000/get-other-posted-jobs-by-this-company/${getCompanyDetails.data.uid}`
        );
  ...
  const handleApplyJob = async () => {
    ...
    const response = await axios.post(
      "http://localhost:5000/set-employee-apply-job",
      { _id, uid }
    ).then(response => {
